...
/**
 * Helper function to convert @lid to @s.whatsapp.net using group metadata
 * @param {Object} conn - WhatsApp connection
 * @param {string} lidId - ID in @lid format
 * @param {string} groupJid - Group JID
 * @returns {Promise<string>} - Converted JID in @s.whatsapp.net format
 */
async function convertLidToJid(conn, lidId, groupJid) {
    try {
        if (!lidId.includes('@lid') || !groupJid.endsWith('@g.us')) {
            return lidId
        }
        
        // Get group metadata to find the mapping
        ...
        // Fallback: convert @lid to @s.whatsapp.net by extracting number
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                // Handle @lid issue - prioritize participantPn or jid over participant 
                let participant = this.participant || this.key.participant || ''
                
                // If participant uses @lid format, try to get the actual WhatsApp number
                if (participant.includes('@lid')) {
                    // Try to get the actual jid from key.participantPn if available (newer Baileys versions)
                    if (this.key?.participantPn) {
                        participant = this.key.participantPn
                    } else if (this.jid) {
                ...
                return this.conn?.decodeJid(this.key?.fromMe && this.conn?.user.id || participant || this.chat || '')
            },
            enumerable: true
        },
        fromMe: {
