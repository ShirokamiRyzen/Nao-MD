...
const handler = async (m, { conn, text, command, usedPrefix }) => {
  if (!text) {
    throw `_*Masukkan URL yang ingin Anda cek!*_\n_*perintah:*_ ${usedPrefix + command} https://api.ryzendesu.com\n*_atau_*\n${usedPrefix + command} api.ryzendesu.com`;
  }

  const url = `https://backend.ryzendesu.com/cj?u=${encodeURIComponent(text)}`;
